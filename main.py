...
config["mptcp_is_enabled"] = False

config["amount_of_bytes_to_request"] = 2 * 1000**3

config["number_of_samples_per_data_point"] = 3

config["client_path_a"] = {}
config["client_path_a"]["bandwidth"] = 500
config["client_path_a"]["delay"] = 0
config["client_path_a"]["packet_loss"] = 0
config["client_path_a"]["queue_size"] = 10

config["client_path_b"] = {}
config["client_path_b"]["bandwidth"] = 500
config["client_path_b"]["delay"] = 0
...
def run_cmd(net, cmd_str):
    ...
    f = open(script_name, 'a+')  # open file in append mode
...
def runConfig():
    ...
    for i in range(config["number_of_samples_per_data_point"]):
        ...
        time.sleep(1)
        res = run_cmd(net, "h1 python3 client.py 2000000000")
        print(res)
